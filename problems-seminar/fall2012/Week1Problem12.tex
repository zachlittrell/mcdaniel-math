...
\begin{enumerate}
  \item For each mathematician, $M$, make two vertices, one for each
        ...
        as $M_1$ and $M_2$.
  \item Two distinct vertices share an edge iff the snooze sessions the        vertices represent overlap.
  \item Given a vertex $v$, $v$ does not share an edge with
        itself.
\end{enumerate}
...
\begin{itemize}
  ...
  \item \textbf{$\oslash$} For two Snooze Vertices, $v$ and $w$,
                           ...
                           If $time(v)$ and $time(w)$ do not overlap,
                           then $v \oslash w = null time$.
  ...
  \item \textbf{$\circledast$} For two Snooze Vertices, $v$ and $w$,
                               ...
                               If $time(v)$ or $time(w)$ are 
                               $null time$ then $v \circledast w$
                               is false.
\end{itemize}
...
\underline{Proof of Theorem 1 by Induction on $n$:}
...
Let $N$ = $L \oplus M$.
Construct a new graph, $G'$, with all vertices of $G$ except$L$ and $M$, and also with $N$, using rules 2 and 3 that we used for $G$.\\
...
\begin{enumerate}
  ...
  \item Without loss of generality, let $X = N$.\\
        ...
        So, there is some span of time where $time(Y \oslash Z)$ 
        overlaps with either $time(L)$ or $time(M)$, or both.\\
